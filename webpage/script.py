...
class Topographical_indices:
    ...
    def high_pass_median_filter_05m(self, input_path):
    ...
    def high_pass_median_filter_1m(self, input_path):
        wbt.high_pass_median_filter(
        i = input_path, 
        output =  self.temp_dir + os.path.basename(input_path),  
        filterx=5, 
...
def main(input_path, out_path_binary, model_path, temp_dir, img_type, tile_size, margin,
         threshold, wo_crf):
    ...
    for img_path in imgs:
        ...
        topographical.high_pass_median_filter_1m(img_path)
        print('High pass median filter created in as', os.path.join(temp_dir, img_path))

    hpmf = [os.path.join(temp_dir, f) for f in os.listdir(temp_dir)
                if f.endswith('.tif')]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--temp_dir', help= 'temp_dir',default='/min/temp_dir/')
